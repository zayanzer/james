...
zokou({nomCom:"mustaffa",reaction:"📡",categorie:"IA"},async(dest,zk,commandeOptions)=>{

  const {repondre,ms,arg}=commandeOptions;
  
    if(!arg || !arg[0])
    {return repondre("HI IF YOU ARE A GIRL MUSTAFFA LOVES YOU👌😘❤️❤️.")}
    //var quest = arg.join(' ');
  try{
...
.catch(error => {
  ...
  zokou({ nomCom: "dalle", reaction: "📡", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      let caption = '*powered by MUSTAFFA-MD*';
