...
zokou({nomCom : "instagram" , categorie : "Download"},async (dest , zk , commandeOptions)=>{
  ...
  try {
    ...
    zk.sendMessage(dest,{video : {url : igvid.data.data.data[0].url},caption : "ig video downloader powered by *mustaffa-Md*",gifPlayback : false },{quoted : ms}) 
    }
    else {
        zk.sendMessage(dest,{image : {url : igvid.data.data.data[0].url},caption : "ig image downloader powered by *mustaffa-Md*"})
    }
...
async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    .then((result) => {
      ...
      `;
       zk.sendMessage(dest,{image : { url : result.thumbnail}, caption : caption},{quoted : ms}) ;
       zk.sendMessage(dest, { video: { url: result.sd  }, caption: 'facebook video downloader powered by *mustaffa-MD*' }, { quoted: ms });
